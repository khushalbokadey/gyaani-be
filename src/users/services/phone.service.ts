...
export class PhoneService {
   ...
   */
  async sendOtpSms(phoneNumber: string, otpCode: string, type: string, firstName?: string): Promise<void> {
    try {
      ...
      this.logger.error('Error sending OTP SMS', error instanceof Error ? error.stack : String(error));
      throw error;
    }
  }
   ...
   */
  private buildOtpMessage(otpCode: string, type: string, firstName?: string): string {
  ...
  async sendWelcomeSms(phoneNumber: string, firstName: string): Promise<void> {
    try {
      ...
      this.logger.error('Error sending welcome SMS', error instanceof Error ? error.stack : String(error));
      throw error;
    }
  }
}
